...
class SelfServer:
    ...
    def generate(self, prompts, sampling_params, enable_thinking):
        ...
        for output in outputs:
            result_list.append([o.text.split("[Code Solution End]")[0].split("[Code Solution]")[-1] for o in output.outputs])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='lcb')
    parser.add_argument('--sampling_mode', type=str, default='qwen3')
    parser.add_argument('--output_file', type=str, default='output.json')
    parser.add_argument('--model_path', type=str, default='qwen3')
    parser.add_argument('--enable_thinking', action='store_true', help='启用慢思考')
    parser.add_argument('--greedy', action='store_true', help='启用慢思考')
    ...
    if args.enable_thinking:
        ...
        sampling_params = SamplingParams(n=args.n, temperature=0, max_tokens=args.max_tokens,stop=["[Code Solution End]"])
    else:
        sampling_params = SamplingParams(n=args.n, temperature=0.7, max_tokens=args.max_tokens)
        # sampling_params = SamplingParams(n=args.n, temperature=0.7, top_p=0.8, top_k=20, presence_penalty=1.5, max_tokens=args.max_tokens)
        # qwen1.5-32b-chat: 
        # sampling_params = SamplingParams(n=args.n, temperature=0.6, max_tokens=args.max_tokens)
    ...
    for i, example_group in enumerate(example_groups):
        ...
        for response, response_with_think, example in zip(responses, responses_with_think, example_group):
            if 1 == 1:  # 多个输出写成多行，而不是放在一个列表中
                for res, rest in zip(response, response_with_think):
                    example[out_key] = res
                    example[out_with_think_key] = rest
                    if example[out_key] != "":
                        # example[out_with_think_key] = rest
                        write_jsonl(args.output_file, example, append=True)
            else:
                example[out_key] = response
                # example[out_with_think_key] = response_with_think
                write_jsonl(args.output_file, example, append=True)
